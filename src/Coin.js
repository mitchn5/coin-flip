...
export default function Coin({ onResult, color = "gold", headsType = "default", tailsType = "default", soundEnabled = true }) {
  ...
  // Create and preload the audio once
  ...
  for (let tex of [headsTex, tailsTex]) {
    tex.anisotropy = 16;
    tex.flipY = true;
  ...
  useEffect(() => {
    const flipHandler = () => {
      ...
      // Play sound
      if (coinSound.current && soundRef.current) {
  ...
  // TODO: Coin maintains some velocity after reset
  useFrame(() => {
...
// Auto-center function for coin drift
